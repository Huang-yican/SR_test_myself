...
#所以放大倍数不同，如放大4倍和放大8倍相比，网络模型在block8的层数会相差一层，所以可以训练多轮数据后，根据实际需要调用相应的网络模型参数文件
    ...
    def forward(self, x):
        ...
        esa1 = self.esa1(block2)
        block3 = self.block3(esa1)
        esa2 = self.esa1(block3)
        block4 = self.block4(esa2)
        esa3 = self.esa3(block4)
        block5 = self.block5(esa3)
        esa4 = self.esa4(block5)
        block6 = self.block6(esa4)
        esa5 = self.esa5(block6)
        block7 = self.block7(esa5)
        esa6 = self.esa6(block7)        
