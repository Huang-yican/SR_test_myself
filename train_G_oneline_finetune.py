...
import pytorch_ssim
from data_utils_G_oneline import TrainDatasetFromFolder, ValDatasetFromFolder, display_transform
...
# 对于微调，有一些改变，lr更小，初始是0.001，每隔5epoch就下降1/3，同时，每隔10epoch就保存结果，loss中是ganloss主导，跑个30epoch先看看
if __name__ == '__main__':
    ...
    schedulerG = StepLR(optimizerG, step_size= 5*num, gamma=1 / 3)
    optimizerD = torch.optim.Adam(netD.parameters(), lr=0.0001)
    schedulerD = StepLR(optimizerD, step_size= 5*num, gamma=1 / 3)
    ...
    for epoch in range(1, NUM_EPOCHS + 1):#遍历num_epoch次
        ...
        netG.eval()#创建文件夹SRF_
        out_path = 'training_results/SRF_reparam_ECB6_ESA6G_DIV2K_G_oneline_imgsize_128_finetune' + str(UPSCALE_FACTOR) + '/'
        ...
        if epoch % 5 == 0:
            torch.save(netG.state_dict(), 'epochs_reparam_ESB6_ESA6G_epoch600_DIV2K_G_oneline_imgsize_128_finetune/netG_epoch_%d_%d.pth' % (UPSCALE_FACTOR, epoch))
            torch.save(netD.state_dict(), 'epochs_reparam_ESB6_ESA6G_epoch600_DIV2K_G_oneline_imgsize_128_finetune/netD_epoch_%d_%d.pth' % (UPSCALE_FACTOR, epoch))
        ...
        torch.save(netG.state_dict(), 'epochs_reparam_ESB6_ESA6G_epoch600_DIV2K_G_oneline_imgsize_128_finetune/netG_last_%d.pth' % (UPSCALE_FACTOR))
        torch.save(netD.state_dict(), 'epochs_reparam_ESB6_ESA6G_epoch600_DIV2K_G_oneline_imgsize_128_finetune/netD_last_%d.pth' % (UPSCALE_FACTOR))
        # save the best model parameter
        if valing_results['psnr'] >= max(results['psnr']):
            torch.save(netG.state_dict(), 'epochs_reparam_ESB6_ESA6G_epoch600_DIV2K_G_oneline_imgsize_128_finetune/netG_best_%d.pth' % (UPSCALE_FACTOR))
            torch.save(netD.state_dict(), 'epochs_reparam_ESB6_ESA6G_epoch600_DIV2K_G_oneline_imgsize_128_finetune/netD_best_%d.pth' % (UPSCALE_FACTOR))
        ...
        if epoch % 20 == 0 and epoch != 0:#每20个epoch后将训练结果写入statistics/csv文件中
            ...
            data_frame.to_csv(out_path + 'srf_' + str(UPSCALE_FACTOR) + '_train_results_reparam_ECB6_ESA6G_epoch600_DIV2K_G_oneline_imgsize_128_finetune.csv', index_label='Epoch')
