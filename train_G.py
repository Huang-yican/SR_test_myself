...
import pytorch_ssim
from data_utils_G import TrainDatasetFromFolder, ValDatasetFromFolder, display_transform
...
if __name__ == '__main__':
    ...
    for epoch in range(1, NUM_EPOCHS + 1):#遍历num_epoch次
        ...
        netG.eval()#创建文件夹SRF_
        out_path = 'training_results/SRF_reparam_ECB6_ESA6G_DIV2K_G_anyline_imgsize_128' + str(UPSCALE_FACTOR) + '/'
        ...
        if epoch % 100 == 0:
            torch.save(netG.state_dict(), 'epochs_reparam_ESB6_ESA6G_epoch600_DIV2K_G_anyline_imgsize_128/netG_epoch_%d_%d.pth' % (UPSCALE_FACTOR, epoch))
            torch.save(netD.state_dict(), 'epochs_reparam_ESB6_ESA6G_epoch600_DIV2K_G_anyline_imgsize_128/netD_epoch_%d_%d.pth' % (UPSCALE_FACTOR, epoch))
        ...
        torch.save(netG.state_dict(), 'epochs_reparam_ESB6_ESA6G_epoch600_DIV2K_G_anyline_imgsize_128/netG_last_%d.pth' % (UPSCALE_FACTOR))
        torch.save(netD.state_dict(), 'epochs_reparam_ESB6_ESA6G_epoch600_DIV2K_G_anyline_imgsize_128/netD_last_%d.pth' % (UPSCALE_FACTOR))
        # save the best model parameter
        if valing_results['psnr'] >= max(results['psnr']):
            torch.save(netG.state_dict(), 'epochs_reparam_ESB6_ESA6G_epoch600_DIV2K_G_anyline_imgsize_128/netG_best_%d.pth' % (UPSCALE_FACTOR))
            torch.save(netD.state_dict(), 'epochs_reparam_ESB6_ESA6G_epoch600_DIV2K_G_anyline_imgsize_128/netD_best_%d.pth' % (UPSCALE_FACTOR))
        ...
        if epoch % 20 == 0 and epoch != 0:#每20个epoch后将训练结果写入statistics/csv文件中
            ...
            data_frame.to_csv(out_path + 'srf_' + str(UPSCALE_FACTOR) + '_train_results_reparam_ECB6_ESA6G_epoch600_DIV2K_imgsize_128.csv', index_label='Epoch')
