...
parser.add_argument('--num_epochs', default=800, type=int, help='train epoch number')#先跑个10轮
...
if __name__ == '__main__':
    ...
    train_set = TrainDatasetFromFolder('data02/train_HR', crop_size=CROP_SIZE, upscale_factor=UPSCALE_FACTOR)
    val_set = ValDatasetFromFolder('data02/val_HR', upscale_factor=UPSCALE_FACTOR)
    train_loader = DataLoader(dataset=train_set, num_workers=4, batch_size=152, shuffle=True)#batch_size改小，原来是64，12用了2.6g，可以考虑20
    ...
    for epoch in range(1, NUM_EPOCHS + 1):#遍历num_epoch次
        ...
        netG.eval()#创建文件夹SRF_
        out_path = 'training_results/SRF_reparam_ECB6_ESA6_bigdatvoc2012_' + str(UPSCALE_FACTOR) + '/'
        ...
        if epoch % 40 == 0:
            torch.save(netG.state_dict(), 'epochs_reparam_ESB6_ESA6_epoch800_voc2012/netG_epoch_%d_%d.pth' % (UPSCALE_FACTOR, epoch))
            torch.save(netD.state_dict(), 'epochs_reparam_ESB6_ESA6_epoch800_voc2012/netD_epoch_%d_%d.pth' % (UPSCALE_FACTOR, epoch))
        ...
        if valing_results['psnr'] >= max(results['psnr']):
            torch.save(netG.state_dict(), 'epochs_reparam_ESB6_ESA6_epoch800_voc2012/netG_best_%d.pth' % (UPSCALE_FACTOR))
            torch.save(netD.state_dict(), 'epochs_reparam_ESB6_ESA6_epoch800_voc2012/netD_best_%d.pth' % (UPSCALE_FACTOR))
        ...
        if epoch % 20 == 0 and epoch != 0:#每20个epoch后将训练结果写入statistics/csv文件中
            ...
            data_frame.to_csv(out_path + 'srf_' + str(UPSCALE_FACTOR) + '_train_results_reparam_ECB6_ESA6_epoch800_voc2012.csv', index_label='Epoch')
