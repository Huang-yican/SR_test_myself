...
from PIL import Image, ImageFilter#Python Imaging Library 的 Image 模块，用于图像的读取、处理、保存等。
...
def flu_transform(inputs):
    # 高斯模糊处理
    inputs = ToPILImage()(inputs)
    blur_PIL = inputs.filter(ImageFilter.GaussianBlur(radius=3))
    blur_tensor = ToTensor()(blur_PIL)
    return blur_tensor
...
class TrainDatasetFromFolder(Dataset):#在创建对象时需要传入一个 dataset_dir、截取图像大小、超分倍数，该目录下应包含所有要用于验证的图像文件。
    ...
    def __getitem__(self, index):
        ...
        #引入高斯卷积
        # hr_image = hr_image.numpy()#调用cv需要输入的是numpy格式   
        blu_image = flu_transform(hr_image)
        # blu_image = ToTensor()(blu_image)#将numpy格式的转换为tensor格式
...
class ValDatasetFromFolder(Dataset):#在创建对象时需要传入一个 dataset_dir，该目录下应包含所有要用于验证的图像文件。
    ...
    def __getitem__(self, index):
        ...
        lr_image = flu_transform(lr_image)
