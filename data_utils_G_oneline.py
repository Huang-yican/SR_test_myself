...
class TrainDatasetFromFolder(Dataset):#在创建对象时需要传入一个 dataset_dir、截取图像大小、超分倍数，该目录下应包含所有要用于验证的图像文件。
    ...
    def __getitem__(self, index):
        ...
        hr_image_line = draw_line(hr_image_crop)
...
if __name__ == '__main__':
    train_dataset = TrainDatasetFromFolder("data_mini/train_HR",128,4)
    ...
    for data, target in train_dataloader:
        ...
        target_img.save("data_mini/test/test_target_oneline{}.jpg".format(i))
        i += 1
        print("yes!")
        print(data.shape)
        print(target.shape)